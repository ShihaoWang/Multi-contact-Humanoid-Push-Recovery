...
// static bool StageStateOptimization( const double & sVal, const double & sUnit, double & sNew, Vector3 & CurContactPos,Robot & SimRobotInner,
//                                     const Config & CurConfig, const Config & CurVelocity,
...
//     sCur+=sCurUnit;
//     sNew = sCur;
//     EndEffectorPathObj.s2Pos(sCur, CurContactPos);
//     Vector3 PlannedCurContactPos = CurContactPos;
//     SimParaObj.setCurContactPos(PlannedCurContactPos);
...
//         FinderFlag = AccPhaseTimePathMethod( CurConfig, NextConfig,
...
//         FinderFlag = DecPhaseTimePathMethod( CurConfig, NextConfig,
//                                             CurVelocity, NextVelocity,
//                                             SimRobotInner.velMax, SimRobotInner.accMax,
...
RecoveryReferenceInfo WholeBodyPlanning(  const Robot & SimRobotInner, const InvertedPendulumInfo & InvertedPendulumInner, 
  ...
  while ((sVal<1.0)&& (!PenetrationFlag)){
    ...
    bool PenetrationFlag = AllPenetrationTester(SimRobot);

  }
}

// ControlReferenceInfo TrajectoryPlanning(const Robot & SimRobotInner, const InvertedPendulumInfo & InvertedPendulumInner, SelfLinkGeoInfo & SelfLinkGeoObj,
...
//   double CurTime = 0.0;
//   Config CurConfig = Config(SimRobotInner.q);
//   Config CurVelocity = SimRobotInner.dq;
//   Vector3 CurContactPos = SimParaObj.getContactInit();
...
//   WholeBodyConfigTraj.push_back(CurConfig);
//   WholeBodyVelocityTraj.push_back(CurVelocity);
//   PlannedEndEffectorTraj.push_back(CurContactPos);
...
//     bool StageOptFlag = StageStateOptimization( sVal, sUnit, sNew, CurContactPos, SimRobotInner, CurConfig, WholeBodyVelocityTraj.back(),
...
//     TimeTraj.push_back(CurTime);
//     WholeBodyConfigAppender(WholeBodyConfigTraj, CurConfig);

...
static Vector3 RotMat2EulerAngles(const Matrix3 & RotMat){
...
static void StepIntegrator(InvertedPendulumInfo & InvertedPendulumObj, const Vector3 & RotAxis, double TimeStep){
...
static std::vector<double> GlobalFrameConfigUpdate(const Robot & SimRobot, const double & ThetaOffset, const Vector3 & RotAxis, const Vector3 & AxisOri){
...
Config WholeBodyDynamicsIntegrator(const Robot & SimRobotInner, const int & SwingLinkInfoIndex, InvertedPendulumInfo & InvertedPendulumObj, const double & TimeDuration, bool & TouchDownFlag, bool & PenetrationFlag){
  TouchDownFlag = PenetrationTester(SimRobotInner, SwingLinkInfoIndex);
  PenetrationFlag = OtherPenetrationTester(SimRobotInner, SwingLinkInfoIndex);
  if(TouchDownFlag) return SimRobotInner.q;
  if(PenetrationFlag) return SimRobotInner.q;
  
  Robot SimRobot    = SimRobotInner;
  Config InitConfig = SimRobotInner.q;
  Config UpdatedConfig = InitConfig;

  Vector3 RotAxis = InvertedPendulumObj.edge_b - InvertedPendulumObj.edge_a;
  RotAxis.setNormalized(RotAxis);
  const int IntergrationStep = 21;
  double TimeStep = TimeDuration/(1.0 * IntergrationStep - 1.0);
  double ThetaInit = InvertedPendulumObj.Theta;

  int IntergrationIndex = 0;
  while ((IntergrationIndex<IntergrationStep)&&(!PenetrationFlag)&&(!TouchDownFlag)){
    SimRobot.UpdateConfig(InitConfig);
    ...
    SimRobot.UpdateConfig(Config(UpdatedConfig));
    TouchDownFlag = PenetrationTester(SimRobot, SwingLinkInfoIndex);
    PenetrationFlag = OtherPenetrationTester(SimRobot, SwingLinkInfoIndex);
    IntergrationIndex++;
  }

...
static double ZCoordinate(double _X, double _Y, const std::vector<Vector3> & ContactPoints){
...
std::vector<PIPInfo> PIPGenerator( const Vector3 & COMPos, const Vector3 & COMVel, const std::vector<Vector3> & ContactPoints){
  ...
  if(!FlatObj.getFacetValidFlag()) return PIPTotal;
    ...
    for (int i = 0; i < FlatObj.FacetEdges.size(); i++){
      ...
      PIPInfo PIPObj = PIPGeneratorInner(COMPos, COMVel, FirstEdge, SecondEdge);
      PIPTotal.push_back(PIPObj);

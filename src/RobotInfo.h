...
struct CubicSplineInfo{
  CubicSplineInfo(){};
  CubicSplineInfo(const std::vector<Vector3> & pts){
    const int n = pts.size();
    std::vector<double> S(n);
    double sUnit = 1.0/(1.0 * n - 1.0);
  ...
  CubicSplineInfo(const tk::spline & _spline_x,
                  const tk::spline & _spline_y,
                  const tk::spline & _spline_z){
    spline_x =_spline_x;
    spline_y =_spline_y;
    spline_z =_spline_z;  }
...
struct EndEffectorPathInfo{
  ...
  EndEffectorPathInfo(){};
  EndEffectorPathInfo(const CubicSplineInfo  & _CubicSplineInfoObj){
    CubicSplineInfoObj = _CubicSplineInfoObj;
    GoalContactPos = CubicSplineInfoObj.s2Pos(1.0);
  }
  void s2Pos(double s, Vector3 & Pos){
    // Here this function is used to get the corresponding position given the total s.
    if(s<0.0) s = 0.0;
    if(s>=1.0) s = 1.0;
    Pos = CubicSplineInfoObj.s2Pos(s);
  }
  CubicSplineInfo CubicSplineInfoObj;
